...
import safex_sendingFunds_utils from '@mysafex/mysafex-sendfunds-utils'
...
class SendFundsView extends View {
  ...
  _setup_camera_scannerLayer () {
    ...
    // {
    // 	const tooltipText = `Drag &amp; drop QR codes<br/>to auto-fill.<br/><br/>Please double-check<br/>your recipient info as<br/>Safex transfers are<br/>not yet&nbsp;reversible.`
  ...
  _setup_form_walletSelectLayer () {
    ...
    const div = commonComponents_forms.New_fieldContainerLayer(self.context)
    {
      ...
      {
        const tooltipText = 'Safex makes transactions<br/>with your "available outputs",<br/>so part of your balance will<br/>be briefly locked and then<br/>returned as change.'
        const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
        const layer = view.layer
        labelLayer.appendChild(layer) // we can append straight to labelLayer as we don't ever change its innerHTML
      }
  ...
  _setup_form_amountInputLayer (tr) {
    ...
    {
      const tooltipText = 'Based on Safex network<br/>fee estimate (not final).<br/><br/>MySafex does not charge<br/>a transfer service fee.'
      const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
      const layer = view.layer
      breakingDiv.appendChild(layer)
    }
  ...
  _setup_form_contactOrAddressPickerLayer () { // Request funds from sender
    ...
    { // resolved safex address field
      ...
      {
        const labelLayer = commonComponents_forms.New_fieldTitle_labelLayer('SAFEX ADDRESS', self.context)
    ...
    div.style.display = 'none' // initial
    {
      ...
      labelRowContainer.style.margin = '0 0 8px 0'
      {
        ...
        const generateButtonView = commonComponents_tables.New_clickableLinkButtonView(
          ...
          self.context,
          function () {
            self.manualPaymentIDInputLayer.value = self.context.safex_utils.new_payment_id()
          }
        )
  ...
  _setup_form_field_priority () {
    ...
    const div = commonComponents_forms.New_fieldContainerLayer(self.context)
    {
      ...
      {
        const tooltipText = 'You can pay the Safex<br/>network a higher fee to<br/>have your transfers<br/>confirmed faster.'
        const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
        const layer = view.layer
        labelLayer.appendChild(layer) // we can append straight to labelLayer as we don't ever change its innerHTML
      }
      ...
      {
        const defaultValue = safex_sendingFunds_utils.default_priority()
        const values =
				[
				  1,
				  2,
  ...
  _setup_qrCodeInputs_containerView () {
    ...
    { // label in contentView
      ...
      //
      div.innerHTML = 'Drag and drop a<br/>Safex Request Code '
      self.qrCodeInputs_contentView.layer.appendChild(div)
    }
    self.addSubview(view)
  }
  ...
  new_xmr_estFeeAmount () {
    const self = this
    const estimatedNetworkFee_JSBigInt = new JSBigInt(self.context.safex_utils.estimated_tx_network_fee(
  ...
  { // may return nil if a wallet isn't present yet
    ...
    const estNetworkFee_safexAmount = self.new_xmr_estFeeAmount()
    const possibleMax_safexAmount = availableWalletBalance.subtract(estNetworkFee_safexAmount)
    if (possibleMax_safexAmount > 0) { // if the Max amount is greater than 0
      return possibleMax_safexAmount
    }
  ...
  new_displayCcyFormatted_estMaxAmountString () { // this is going to return nil if the rate is not ready for the selected display currency - user will probably just have to keep hitting 'max'
    ...
    const xmr_estMaxAmount_str = safex_amount_format_utils.formatMoney(xmr_estMaxAmount)
    //
  ...
  _new_estimatedNetworkFee_displayString () {
    ...
    let finalizable_formattedAmountString = estimatedTotalFee_str// `${estimatedTotalFee_safexAmountDouble}`
    {
      if (displayCcySymbol != Currencies.ccySymbolsByCcy.XMR) {
        const displayCurrencyAmountDouble_orNull = Currencies.displayUnitsRounded_amountInCurrency(
          self.context.CcyConversionRates_Controller_shared,
          displayCcySymbol,
          estimatedTotalFee_safexAmountDouble
        )
        if (displayCurrencyAmountDouble_orNull != null) {
          const displayCurrencyAmountDouble = displayCurrencyAmountDouble_orNull
    ...
    const raw_amount_String = self.amountInputLayer.value
    {
      if (typeof raw_amount_String !== 'undefined' && raw_amount_String) {
        ...
        if (!isNaN(rawInput_amount_Number)) {
          const submittableSafexAmountDouble_orNull = Currencies.submittableSafexAmountDouble_orNull(
            self.context.CcyConversionRates_Controller_shared,
            selected_ccySymbol,
            rawInput_amount_Number
          )
          if (submittableSafexAmountDouble_orNull == null) { // amt input exists but no converted amt found
            if (selected_ccySymbol == Currencies.ccySymbolsByCcy.XMR) {
              throw 'null submittableSafexAmountDouble_orNull while selected_ccySymbol=.XMR'
            }
  ...
  set_effectiveAmountLabelLayer_needsUpdate () {
    ...
    //
    const xmrAmountDouble_orNull = Currencies.submittableSafexAmountDouble_orNull(
      self.context.CcyConversionRates_Controller_shared,
      selected_ccySymbol,
      rawInput_amount_Number
    )
    ...
    if (selected_ccySymbol == XMR) {
      ...
      const safexAmountDouble_atomicPlaces = xmrAmountDouble * Math.pow(10, safex_config.coinUnitPlaces)
      const safexAmount = new JSBigInt(safexAmountDouble_atomicPlaces)
      const formatted_safexAmount = safex_amount_format_utils.formatMoney(safexAmount)
      finalizable_text = `= ${formatted_safexAmount} ${Currencies.ccySymbolsByCcy.XMR}`
    }
  ...
  _clearForm () {
    ...
    }
    {
      ...
      const defaultVal = safex_sendingFunds_utils.default_priority()
  ...
  _tryToGenerateSend () {
    ...
    {
      if (typeof wallet === 'undefined' || !wallet) {
        _trampolineToReturnWithValidationErrorString('Please create a wallet to send Safex.')
        return
      }
    }
    ...
    if (!sweeping) {
      ...
      if (submittableSafexAmountDouble_orNull == null) {
        throw 'submittableSafexAmountDouble unexpectedly null while sending - button should be disabled'
      }
      const submittableSafexAmountDouble = submittableSafexAmountDouble_orNull
      if (submittableSafexAmountDouble <= 0) { // check this /after/ conversion to also check ->0 converted values
        _trampolineToReturnWithValidationErrorString('The amount to send must be greater than zero.')
        return
      }
      final_XMR_amount_Number = submittableSafexAmountDouble
    }
    //
    ...
    if (!sweeping && selected_ccySymbol != Currencies.ccySymbolsByCcy.XMR) {
      const hasAgreedToUsageGateTerms = self.context.settingsController.invisible_hasAgreedToTermsOfCalculatedEffectiveSafexAmount || false
        ...
        self.context.windowDialogs.PresentQuestionAlertDialogWith(
          ...
          function (err, didChooseYes) {
            ...
            self.context.settingsController.Set_settings_valuesByKey(
              {
                invisible_hasAgreedToTermsOfCalculatedEffectiveSafexAmount: true
              },
              function (err) {
                if (err) {
                  throw err
  ...
  _didPickContact (contact) {
    ...
    self.requestHandle_for_oaResolution = self.context.openAliasResolver.ResolveOpenAliasAddress(
      ...
      function (
        ...
        self.set_isSubmittable_needsUpdate()
        {
          if (typeof safexReady_address !== 'undefined' && safexReady_address) { // not that it would be
            self._displayResolvedAddress(safexReady_address)
          } else {
  ...
  _didFinishTypingInContactPickerInput (optl_event) {
    ...
    if (hasEmojiCharacters) {
      const isYat = yatSafexLookup.isValidYatHandle(enteredPossibleAddress)
      self.isYatHandle = isYat
      if (isYat) {
        const lookup = yatSafexLookup.lookupSafexAddresses(enteredPossibleAddress).then((responseMap) => {
          // Our library returns a map with between 0 and 2 keys
          if (responseMap.size == 0) {
            // no safex address
            const errorString = `There is no Safex address associated with "${enteredPossibleAddress}"`
            self.validationMessageLayer.SetValidationError(errorString)
          } else if (responseMap.size == 1) {
            // Either a Safex address or a Safex subaddress was found.
            ...
            const safexAddress = responseMap.get('0x1001')
            self._displayResolvedAddress(safexAddress)
          }
        }).catch((error) => {
    ...
    self.set_isSubmittable_needsUpdate()
    //
    const isOAAddress = safex_openalias_utils.DoesStringContainPeriodChar_excludingAsXMRAddress_qualifyingAsPossibleOAAddress(enteredPossibleAddress)
    if (isOAAddress !== true) {
      let address__decode_result
      try {
        address__decode_result = self.context.safex_utils.decode_address(enteredPossibleAddress, self.context.nettype)
      } catch (e) {
    ...
    self.requestHandle_for_oaResolution = self.context.openAliasResolver.ResolveOpenAliasAddress(
      ...
      function (
        ...
        //
        if (typeof safexReady_address !== 'undefined' && safexReady_address) {
          self._displayResolvedAddress(safexReady_address)
        } else {
          // we already hid it above
        }
    ...
    img.addEventListener(
      ...
      function () {
        ...
        if (!code || !code.location) {
          self.validationMessageLayer.SetValidationError('MySafex was unable to find a QR code in that image.')
          return
        }
        const stringData = code.data
        if (!stringData) {
          self.validationMessageLayer.SetValidationError('MySafex was unable to decode a QR code from that image.')
          return
        }
        if (typeof stringData !== 'string') {
          self.validationMessageLayer.SetValidationError('MySafex was able to decode QR code but got unrecognized result.')
          return
        }
    ...
    if (typeof (possibleUriString) !== 'undefined') {
      ...
      try {
        if (possibleUriString.length > 0) {
          requestPayload = safex_requestURI_utils.New_ParsedPayload_FromPossibleRequestURIString(possibleUriString, self.context.nettype, self.context.safex_utils)
        }
      } catch (errStr) {
        if (errStr) {
