...
class UseExisting_MetaInfo_View extends BaseView_Wallet_MetaInfo {
  ...
  _setup_form_walletMnemonicField () {
    ...
    div.style.paddingBottom = '0' // instead of 20, here, special case... we will move the 20 to the "Or, use…" layer
    {
      ...
      {
        const tooltipText = 'This secret mnemonic is never<br/>sent to the MySafex server.'
  ...
  _setup_form_walletAddrAndKeysFields () {
    ...
    { // wallet viewKey
      ...
      div.style.paddingBottom = '0' // instead of 20, here, special case... we will move the 20 to the "Or, use…" layer
      {
        ...
        div.appendChild(labelLayer)
        {
          const tooltipText = 'This private view key and the wallet<br/>address are the only things sent<br/>to the MySafex server.'
          const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
          const layer = view.layer
          labelLayer.appendChild(layer)
        }
    ...
    { // wallet spendKey
      ...
      div.style.paddingBottom = '0' // instead of 20, here, special case... we will move the 20 to the "Or, use…" layer
      {
        ...
        div.appendChild(labelLayer)
        {
          const tooltipText = 'This private spend key is never<br/>sent to the MySafex server.'
          const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
          const layer = view.layer
          labelLayer.appendChild(layer)
        }
  ...
  _userSelectedNextButton () {
    const self = this
    {
      ...
      //
      try {
        const ret = self.context.safex_utils.seed_and_keys_from_mnemonic(
