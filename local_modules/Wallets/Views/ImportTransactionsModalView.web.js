...
class ImportTransactionsModalView extends View {
  ...
  _tryToGenerateSend () {
    ...
    wallet.SendFunds(
      ...
      false, // sweeping
      safex_sendingFunds_utils.default_priority(),
      //
  ...
  viewDidAppear () {
    ...
    if (self.requestHandle_for_importRequestInfoAndStatus == null || typeof self.requestHandle_for_importRequestInfoAndStatus === 'undefined') {
      const requestHandle = self.context.hostedSafexAPIClient.ImportRequestInfoAndStatus(
        ...
        function (
          ...
          const raw_formattedMoney = safex_amount_format_utils.formatMoney(import_fee__JSBigInt)
          {
            self.informationalHeaderLayer.innerHTML = `This requires a one-time import fee of ${raw_formattedMoney} XMR`
            //
            const tooltipText = `Importing your wallet means the server will scan the entire Safex blockchain for your wallet's past transactions, then stay up-to-date.<br/><br/>As this process places heavy load on the server, import is triggered by sending a fee (e.g. from the original wallet) with the specific payment ID below to the server at e.g. ${self.approximate_importOAAddress}.`
