...
//
const safex_txParsing_utils = require('@mysafex/mysafex-tx-parsing-utils')
const safex_sendingFunds_utils = require('@mysafex/mysafex-sendfunds-utils')
const JSBigInt = require('@mysafex/mysafex-bigint').BigInteger
const safex_amount_format_utils = require('@mysafex/mysafex-money-format')
const safex_config = require('@mysafex/mysafex-safex-config')
const mnemonic_languages = require('@mysafex/mysafex-locales')
//
...
class Wallet extends EventEmitter {
  ...
  __setup_andAwaitBootAndLogInAndDocumentCreation () {
    ...
    function _createWithLocale (currentLocale/* TODO rename */) {
      ...
      //
      // NOTE: the wallet needs to be imported to the hosted API (e.g. MySafex) for the hosted API stuff to work
      ...
      try {
        const ret = self.context.safex_utils.newly_created_wallet(
        ...
        self.generatedOnInit_walletDescription =
				{ // this structure here is an artifact of a previous organization of the mysafex-core-js code. it should/can be phased out
  ...
  Boot_byLoggingIn_existingWallet_withMnemonic (
    ...
    let ret
    try {
      ret = self.context.safex_utils.seed_and_keys_from_mnemonic(
        mnemonicString,
        self.context.nettype
      )
    } catch (e) {
  ...
  _trampolineFor_successfullyBooted (
    ...
    const self = this
    {
      if (typeof self.account_seed === 'undefined' || self.account_seed === null || self.account_seed == '') {
        ...
        const derived_mnemonicString = self.context.safex_utils.mnemonic_from_seed(self.account_seed, self.mnemonic_wordsetName)
        if (self.mnemonicString != null && typeof self.mnemonicString !== 'undefined') {
          const areMnemonicsEqual = self.context.safex_utils.are_equal_mnemonics(
            self.mnemonicString,
  ...
  _boot_byLoggingIn (
    ...
    let ret
    try {
      ret = self.context.safex_utils.validate_components_for_login(
        address,
        view_key,
    ...
    self.requestHandle_for_logIn = self.context.hostedSafexAPIClient.LogIn(
      address,
      view_key,
      wasAGeneratedWallet,
      function (login__err, new_address, received__generated_locally, start_height) {
        ...
        if (login__err) {
          if (shouldExitOnLoginError == true) {
            ...
            // reboot. reason is that we expect all such wallets to be valid safex
  ...
  IsTransactionConfirmed (tx) {
    ...
    //
    return safex_txParsing_utils.IsTransactionConfirmed(tx, blockchain_height)
  }
  ...
  IsTransactionUnlocked (tx) {
    ...
    const blockchain_height = self.blockchain_height
    //
    return safex_txParsing_utils.IsTransactionUnlocked(tx, blockchain_height)
  }
  ...
  TransactionLockedReason (tx) {
    ...
    const blockchain_height = self.blockchain_height
    //
    return safex_txParsing_utils.TransactionLockedReason(tx, blockchain_height)
  }
  ...
  Balance_FormattedString () { // provided for convenience mainly so consumers don't have to require safex_utils
    const self = this
    const balance_JSBigInt = self.Balance_JSBigInt()
    //
    return safex_amount_format_utils.formatMoney(balance_JSBigInt)
  }
  ...
  LockedBalance_FormattedString () { // provided for convenience mainly so consumers don't have to require safex_utils
    const self = this
    const lockedBalance_JSBigInt = self.LockedBalance_JSBigInt()
    //
    return safex_amount_format_utils.formatMoney(lockedBalance_JSBigInt)
  }
  ...
  AmountPending_FormattedString () { // provided for convenience mainly so consumers don't have to require safex_utils
    const self = this
  ...
  SendFunds (
    ...
    const failureCodeMessage_byEnumVal =
		{
		  ...
		  6: "Couldn't validate destination Safex address.",
    ...
    args.success_fn = function (params) {
      ...
      const total_sent__floatString = safex_amount_format_utils.formatMoney(total_sent__JSBigInt)
      ...
      if (code === 0 || (typeof code === 'undefined' || code === null)) { // msgProvided
        ...
        if (params.createTx_errCode == 90) { // needMoreMoneyThanFound
          errStr = `Spendable balance too low. Have ${
						safex_amount_format_utils.formatMoney(new JSBigInt('' + params.spendable_balance))
					} ${safex_config.coinSymbol}; need ${
						safex_amount_format_utils.formatMoney(new JSBigInt('' + params.required_balance))
					} ${safex_config.coinSymbol}.`
        } else {
    ...
    args.get_unspent_outs_fn = function (req_params, cb) {
      self.context.hostedSafexAPIClient.UnspentOuts(req_params, cb)
    }
    args.get_random_outs_fn = function (req_params, cb) {
      self.context.hostedSafexAPIClient.RandomOuts(req_params, cb)
    }
    args.submit_raw_tx_fn = function (req_params, cb) {
      self.context.hostedSafexAPIClient.SubmitRawTx(req_params, cb)
    }
    self.context.safex_utils.async__send_funds(args)
  }
