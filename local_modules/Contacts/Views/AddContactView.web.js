...
class AddContactView extends ContactFormView {
  ...
  _tryToCreateOrSaveContact () {
    ...
    if (typeof paymentID !== 'undefined' && paymentID) {
      if (self.context.safex_utils.is_subaddress(address, self.context.nettype)) { // paymentID disallowed with subaddress
    ...
    if (openAliasResolver.DoesStringContainPeriodChar_excludingAsXMRAddress_qualifyingAsPossibleOAAddress(address) === false) {
      ...
      try {
        address__decode_result = self.context.safex_utils.decode_address(address, self.context.nettype)
      } catch (e) {
        __reEnableForm()
        self.validationMessageLayer.SetValidationError('Please enter a valid Safex address') // not using the error here cause it can be pretty unhelpful to the lay user
        return
      }
      ...
      if (isIntegratedAddress) { // is integrated address
        ...
        if (self.context.safex_utils.is_subaddress(address, self.context.nettype)) { // paymentID disallowed with subaddress
          ...
          if (paymentID === '' || typeof paymentID === 'undefined') { // if no existing payment ID
            paymentID = self.context.safex_utils.new_payment_id() // generate new one for them
            self.paymentIDInputLayer.value = paymentID
      ...
      self.requestHandle_for_oaResolution = self.context.openAliasResolver.ResolveOpenAliasAddress(
        ...
        function (
          ...
          const cached_OAResolved_XMR_address = safexReady_address
          _proceedTo_addContact_paymentID(
    ...
    function _proceedTo_addContact_paymentID (paymentID__toSave, cached_OAResolved_XMR_address__orUndefined) {
      ...
      const paymentID_existsAndIsNotValid = paymentID_exists && safex_paymentID_utils.IsValidPaymentIDOrNoPaymentID(paymentID__toSave) === false
  ...
  _shared_didPickPossibleRequestURIStringForAutofill (possibleUriString) {
    ...
    if (typeof (possibleUriString) !== 'undefined') {
      ...
      let parsedPayload
      try {
        parsedPayload = safex_requestURI_utils.New_ParsedPayload_FromPossibleRequestURIString(possibleUriString, self.context.nettype, self.context.safex_utils)
      } catch (errStr) {
        if (errStr) {
